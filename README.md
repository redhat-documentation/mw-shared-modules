...
3. Use the `asciidoc-coalescer` script to generate a new assembly with the include directives resolved.

   This script processes the assembly in which you added the remote include to the shared module(s), resolves the include directives, and generates a separate AsciiDoc file with all of the content.

   For example, this command coalesces the content in `my-original-assembly` and generates a new assembly that can be processed by CCS tooling (`my-generated-assembly`):

   ```
   mw-shared-modules/scripts/asciidoc-coalescer.rb -a allow-uri-read my-new-assembly.adoc my-generated-assembly.adoc
   ```

4. To use the assembly in your book, include the generated assembly in your book's `master.adoc` file.

   **NOTE:** You must include the assembly that was generated by `asciidoc-coalescer.rb`, *not* the original assembly in which you added the remote include.
